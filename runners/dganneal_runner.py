...
__all__ = ['DGAnnealRunner']


class DGAnnealRunner():
    ...
    def train(self):
        ...
        elif self.config.data.dataset == 'CELEBA-BLOND':
            dataset, _, test_dataset = get_datasets(self.args.data_dir,'CelebA_Blond',[self.config.data.target_domain],val=0,hparams={"data_augmentation": True, "image_size": self.config.data.image_size})
        
        elif self.config.data.dataset == 'VLCS':
            dataset, _, test_dataset = get_datasets(self.args.data_dir,'VLCS',[self.config.data.target_domain],val=0,hparams={"data_augmentation": True, "image_size": self.config.data.image_size})
        
        elif self.config.data.dataset == 'PACS':
            dataset, _, test_dataset = get_datasets(self.args.data_dir,'PACS',[self.config.data.target_domain],val=0,hparams={"data_augmentation": True, "image_size": self.config.data.image_size})
        
        elif self.config.data.dataset == 'NICO':
            dataset, _, test_dataset = get_datasets(self.args.data_dir,'NICO_Mixed',[self.config.data.target_domain],val=0,hparams={"data_augmentation": True, "image_size": self.config.data.image_size})
        ...
        tb_path = os.path.join(self.args.run, 'tensorboard',  self.config.data.dataset, self.args.doc)
    ...
    def test(self, iters=None):
        ...
        if self.config.data.dataset == 'MNIST':
            ...
            all_samples = self.anneal_Langevin_dynamics(samples, score, sigmas, self.config.training.n_steps_each, 0.00002)
